...
void SORT_RANKED_FILES ( RANKED_FILE* , int , int ) ;
...
int main ()
{
	...
	char PRE_HEADER[] = "| RANK | FILE ID | RANK QUOTIENT |" ;
	int PRE_HL = strlen(PRE_HEADER) ;
	CRAWL_S = clock() ;
	...
	while( true )
	{
		...
		RANKED_FILE* FILES = ( RANKED_FILE* ) calloc ( N , sizeof(RANKED_FILE) ) ;
		for ( int i=0 ; i<N ; i++ )		{ FILES[i].ID = i+1 ; FILES[i].RANK_QUOTIENT = 0.0 ; }
		for ( int i=0 ; i<COUNT ; i++ )
			FILES[R[i].ID-1].RANK_QUOTIENT += (R[i].FREQ) / float(R[i].RELEVANCE+1) ;
		SORT_RANKED_FILES( FILES , 0 , N-1 ) ;	
		
		printf("+") ;
		for ( int i=1 ; i<PRE_HL-17 ; i++ ) printf("--") ;
		printf("+") ;
		printf( "\n%s" , PRE_HEADER ) ;
		
		for ( int i=0 ; i<N ; i++ )
		{
			if ( FILES[i].RANK_QUOTIENT <= 10e-7 )	break ;
			if ( FILES[i].RANK_QUOTIENT >= 10.0 )
				printf( "\n|  %02d  |    %02d   |    %.4f    | " , i+1 , FILES[i].ID , FILES[i].RANK_QUOTIENT ) ;
			else
				printf( "\n|  %02d  |    %02d   |     %.4f    | " , i+1 , FILES[i].ID , FILES[i].RANK_QUOTIENT ) ;
		}
		printf("\n+") ;
		...
		printf("+") ;
		SEARCH_E = clock() ;
		printf ( "\n SEARCHED AND PRINTED %d RESULTS IN %lf SECONDS " , COUNT , (double)(SEARCH_E-SEARCH_S)/(double)CLOCKS_PER_SEC ) ;
	}
}

void SORT_RANKED_FILES ( RANKED_FILE* arr , int si , int ei )
{
	if ( si >= ei ) return ;
	float pivot = arr[si].RANK_QUOTIENT ;
	int i = si ;
	int j = si + 1 ;
	while ( j<=ei )
	{
		if ( arr[j].RANK_QUOTIENT - pivot > 10e-5  )
		{
			i++ ;
			RANKED_FILE t = arr[i] ;
			arr[i] = arr[j] ; arr[j] = t ;
	...
	RANKED_FILE t = arr[i] ;
	arr[i] = arr[si] ; arr[si] = t ;
	SORT_RANKED_FILES( arr , si , i-1 ) ;
	SORT_RANKED_FILES( arr , i+1 , ei ) ;
}
